...
export async function loadPreviewScene() {
  ...
  if (result.ok) {
    ...
    const parcelScene = new UnityParcelSceneHandler(ILandToLoadableParcelScene(defaultScene))
    ...
    if (lastId) {
      globalDCL.unityInterface.UnloadScene(lastId)
    }
    globalDCL.unityInterface.LoadParcelScenes([target])
